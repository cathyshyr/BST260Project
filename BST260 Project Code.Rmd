...
```{r}
set.seed(123)
test_ind <- sample(1:nrow(data_wide), round(nrow(data_wide)*0.2), replace = FALSE)
train_ind <- -test_ind
train_df <- data_wide[train_ind, ]
test_df <- data_wide[test_ind, ]
...
MSE(test_df_lm$Truth, test_df_lm$BRef_lmpred)
MSE(test_df_lm$Truth, test_df_lm$BBpro_lmpred)
MSE(test_df_lm$Truth, test_df_lm$FanGraph_lmpred)
```
